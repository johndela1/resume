...
of trees.  Each tree represented an entity or topic in the graph.  The
cache provided write operations which made sure local
changes where available immediately and pushed back to the remote database.
...
added to scheduled events based on their network (CBS, FOX, etc)
and their start time.
Every 24 hours the schedules would get updated.  This could mean events
where removed, added, or moved.  My system would try to reattach
content for shows that moved in time (if a match could be guessed) and
flag all other cases of disassociated content for human intervention.
