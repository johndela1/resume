...
\begin{resume}
  ...
  \item
      programming: My favorite thing to do.  My goals are to manage
      complexity.  This means using the correct levels of abstraction,
      not coupling things that although seem related are not, using
      good/consistent names.  I strive to write code that is mostly unit
      testable in seconds and the rest can be done with integration
      tests which are a better fit for testing stuff that does IO.
      This keeps tests fast and removes the need for mocking.
  \item
      technology: HTTP, RDBMS, DNS, Terraform, Linux (user and
      kernel), AWS, Lambda, ASG, ECS, ElasticCache (Redis), Kinesis,
      Route53, etc
  \item
      experienced with interviewing and mentoring
  \end{itemize}
